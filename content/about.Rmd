...
* Personal website and second blog created using blogdown and R studio. Presented through a github repository, this website includes plenty of coding basics such as images, hyperlinks, bold and italicized text, headers, centered and indented text, emojis, and a footer link to my Github profile. 

* Blog posts are shown chronologically on the about page, as well as sorted by tag and category. A projects page include 10 various projects, including class notes for 3 of my upper division undergraduate statistics classes.  

* Currently 6 blog posts are live which include brief tutorials on R studio packages such as Jasmines, and tidy census, as well as mathematical proofs, and micro posts on simple but important mathematical and statistical concepts. 

* Quick to construct, approximately one hour is spent weekly making minor adjustments to the overal blog or working on potential or existing posts. 

* Future blog posts will include examples in R from simple linear regression, proving least squares estimates, blogdown tutorial, first shiny app, simple shiny tutorial, probability distributions (deep dive), tree diagrams, bayes theorem, simple g.i.s., basketball stats, and spotifyr. 

**:sunny: [Rsite2](https://rbolt13.github.io/rsite2/)** <br> Summer 2021 <br> _Personal_

* Inspired by riste, rsite2 is a website created mainly using R Studio, and presented on Github. Each page has a different background which was done with a very basic CSS. 

* Images and text with headers are used to show / explain how I created websites like the ones I created for rsite2, rsite, and my classnotes. 

* Created a 5 by 13 data frame of classical ballets, and created two visualizations using ggplot(). The first visual is a plot diagram showing ballets in chronilogical order with La Sylphide being the oldest from 1832, and Alice's Adventures in WonderLand being the newest from 2011. The second visual is a histogram showing that most ballet's have 3 acts, with a range of acts from 1 - 4. 

* First homework solutions from Statistics - 451 : Applied Statistics for Engineers and Scientists, but done in r. Functions used incldue : stem(), hist(), mean(), median(), sort(), sum(), length(), sd(), sqrt(), quantile(), min(), max(), par(), and boxplot()
...
**Topics Covered :** simple linear regression, multiple linear models, model adequacy checking, transformations and weighting to correct model inadequacies, and diagnosstics for leverage and influence 
